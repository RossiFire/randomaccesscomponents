		...
		<DemoBlock className="m-0" containerClassName="py-12">
			<div className="flex flex-col w-full items-center justify-around space-y-20">
				...
				<div className="w-full gap-4 flex justify-center">
					<MouseFollowContent asChild>
						<div className="flex cursor-pointer flex-col w-60 gap-4 bg-gradient-to-br from-background to-muted-foreground/10 border border-muted-foreground/10 rounded-lg p-4">
							<span className="text-base text-primary">Projects</span>
					...
					<MouseFollowContent asChild>
						<div className="flex cursor-pointer flex-col w-60 gap-4 bg-gradient-to-br from-background to-muted-foreground/10 border border-muted-foreground/10 rounded-lg p-4">
							<span className="text-base text-primary">About</span>
							<span className="text-sm text-muted-foreground">
				...
				<div className="w-full gap-4 flex justify-center">
					<MouseFollowContent asChild>
						<div className="flex cursor-pointer flex-col w-60 gap-4 bg-gradient-to-br from-background to-muted-foreground/10 border border-muted-foreground/10 rounded-lg p-4">
							<span className="text-base text-primary">Icons</span>
							<span className="text-sm text-muted-foreground">
					...
					<MouseFollowContent asChild>
						...
						>
							<span className="text-base text-primary">Dynamic</span>
							<span className="text-sm text-muted-foreground">
								You can play with it to create funny effects like this (Enter and leave the card
								more than once)
							</span>
				...
				<div className="w-full gap-4 flex justify-center">
					<MouseFollowContent asChild>
						...
						>
							<span className="text-base text-primary">Center</span>
							<span className="text-sm text-muted-foreground">
					...
					<MouseFollowContent asChild>
						<div className="flex flex-col w-60 gap-4 bg-gradient-to-br from-background to-muted-foreground/10 border border-muted-foreground/10 rounded-lg p-4">
							<div className="flex flex-col gap-2">
								<span className="text-base text-primary">Top left</span>
							</div>
							<span className="text-sm text-muted-foreground">
								{" "}
...
const FollowMouseItemContent = ({ label }: { label: string }) => {
	return (
		<div className="rounded-lg text-xs bg-muted-foreground/10 backdrop-blur-sm border border-muted-foreground/10 py-1 px-2">
			<div className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-accent-foreground">
				{label}
			</div>
		</div>
	);
...
const CenterItemContent = ({ isVolumeOn }: { isVolumeOn: boolean }) => {
	return (
		<div
			className={cn(
				"rounded-full aspect-square text-xs p-6 border-2 border-dashed border-muted-foreground/20 transition-all duration-150",
				isVolumeOn && "border-primary/50 text-primary"
			)}
		>
			<Volume2Icon
				className={cn(
					"size-4 text-primary transition-all duration-150 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2",
					!isVolumeOn && "scale-0 opacity-0"
				)}
			/>
			<VolumeOffIcon
				className={cn(
