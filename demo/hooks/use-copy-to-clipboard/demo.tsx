...
const CopyToClipboardDemo: React.FC = () => {
	const { copy, isCopied, copiedText } = useCopyToClipboard({ resetAfter: 3000 });
				...
				<div className="text-center flex flex-col gap-4 items-center">
					<div className="flex gap-2 items-center justify-center">
						...
						<Badge
							...
							className={cn(isCopied && "bg-green-500")}
						>
							{isCopied ? "Copied" : "Not Copied"}
						</Badge>
					</div>
					<div className="text-lg font-semibold text-foreground">Copy to clipboard hook</div>
					<div className="flex gap-2 items-center">
						<Button onClick={handleCopyStaticText}>Copy "Hello, World!"</Button>
						<Button onClick={handleCopyCode}>Copy Code Block</Button>
					</div>
					{copiedText && (
						<Alert variant="success" className="rounded-md mx-auto max-w-md p-4 mt-8">
							{copiedText}
						</Alert>
					)}
				</div>
