...
export function useQuickFormHistory<T extends Record<string, any>>(
	...
	const addToHistory = useCallback(
		(data: T) => {
			...
			// Skip first rerender
			if (lastTrackedData.current && !hasDataChanged(lastTrackedData.current, data)) {
				return;
			}
	...
	const undo = useCallback(() => {
		...
		setRedoStack((prev) => [...prev, { data: { ...currentData }, timestamp: Date.now() }]);
	...
	const redo = useCallback(() => {
		...
		setUndoStack((prev) => [...prev, { data: { ...currentData }, timestamp: Date.now() }]);
