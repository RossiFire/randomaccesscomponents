...
const deepEqual = <T extends Record<string, any>>(obj1: T, obj2: T): boolean => {
...
export const useQuickForm = <T extends Record<string, any>>(
	 ...
	 */
	const isZodSchema = useCallback((option: ValidationOption<T>): option is ZodSchema<T> => {
	...
	const validateForm = useCallback(() => {
		// Check cache first
		if (validationCacheRef.current && deepEqual(validationCacheRef.current.data, data)) {
		...
		<K extends keyof T>(field: K, value: T[K] extends (infer U)[] ? U : never) => {
			setFormData((prev) => {
				...
				if (!Array.isArray(currentArray)) {
					console.warn(`Field '${String(field)}' is not an array. Cannot toggle value.`);
					return prev;
				}
