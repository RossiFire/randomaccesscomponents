 ...
 */
type ExtractFormDataType<T> = T extends React.RefObject<FormStateRef<infer U> | null> ? U : never;
 ...
 * ```
 */
export function useQuickFormAnalytics<T extends React.RefObject<FormStateRef<any> | null>>(
	formRef: T
): UseQuickFormAnalyticsReturn {
	...
	const trackFocus = useCallback((field: string) => {
		...
		fieldInteractionsRef.current[field] = (fieldInteractionsRef.current[field] || 0) + 1;
	...
	const trackBlur = useCallback(
		(field: string) => {
			...
			if (focusTime) {
				...
				timeOnFieldRef.current[field] = (timeOnFieldRef.current[field] || 0) + duration;
	...
	useEffect(() => {
		...
		Object.keys(currentErrors).forEach((field) => {
			...
			if (hasError && !hadError) {
				fieldErrorCountRef.current[field] = (fieldErrorCountRef.current[field] || 0) + 1;
				fieldTotalCountRef.current[field] = (fieldTotalCountRef.current[field] || 0) + 1;
	...
	const analytics = useMemo((): FormAnalytics => {
		...
		const filledFields = allFields.filter((key) => {
			...
			const hasError = (formState.errors as Record<string, string | undefined>)[key];
		...
		const completionRate = allFields.length > 0 ? filledFields.length / allFields.length : 0;
