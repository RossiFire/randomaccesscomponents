...
<PreviewCodeBlock preview={<AnimatedTextBlockDemo />}>
    ```tsx
...
<PkgInstallerTabs command="gsap" />


Copy and paste the following code into your project.


<SingleCodeBlock icon={<React />} label="animated-text-block.tsx" lang="tsx">
```tsx
"use client";
...
const TextLineReveal: React.FC<{ children: React.ReactNode }> = ({
  ...
  return (
    <span
      className={cn(
        "faded-text text-2xl md:text-4xl lg:text-6xl font-bold uppercase relative text-foreground leading-[60px] md:leading-[100px] -tracking-wider",
        interFont
      )}
    >
      {children}
...
```tsx title="animated-text-block.tsx" icon="react"
return (
    <AnimatedTextBlock />
)
```
