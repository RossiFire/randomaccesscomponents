...
<PageBadge badges={['tailwindcss', 'css', 'react', 'nextjs']} />
...
## Demo
<PreviewCodeBlock preview={<MarkeeDemo />}>
    ```tsx
const BADGES = [
  ...
  return <MarkeeContentContext.Provider value={true}>
    ...
    >
      <ul
        ...
        role="list"
        aria-label="Markee content list"
        {...props}
      />
...
<PreviewCodeBlock preview={<MarkeeDemoWithGSAP />}>
```tsx
...
<PreviewCodeBlock preview={<MarkeeDemoWithFramerMotion />}>
    ```tsx
