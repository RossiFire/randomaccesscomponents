...
<PkgInstallerTabs command="@supabase/supabase-js @supabase/ssr" />
...
<Tabs defaultValue="nextjs">
  ...
  <TabsContent value="nextjs">
    ```typescript title="getSupabaseAuth.ts" icon="ts"
    ...
    * @returns {GetAuthProps} Auth object {@link GetAuthProps}
    */
    export const getAuth = async (): Promise<GetAuthProps> => {
        const cookie = cookies();
        const supabase = createClient(cookie);
...
async function myServerFunction(){
    ...
    if (!isLoggedIn) {
        const isRandomUser = user.fullName === "Random Access Components" //TS ERROR: since "isLoggedIn" is false, user is undefined
    }  

    const isRandomUser = user.fullName === "Random Access Components" //OK: since "isLoggedIn" is true, user is defined and we can access its properties
