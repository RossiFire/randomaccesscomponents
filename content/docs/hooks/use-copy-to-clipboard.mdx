...
<PreviewCodeBlock preview={<CopyToClipboardDemo />}>
```tsx
'use client'
    ...
    <div className='flex flex-col items-center justify-center gap-4 size-full'>
      <div className="text-center flex flex-col gap-4 items-center">
        <div className='flex gap-2 items-center justify-center'>
          <span>Status:</span>
          <span className={cn(isCopied && 'bg-green-500')}>{isCopied ? 'Copied' : 'Not Copied'}</span>
        </div>
        ...
        {copiedText && <span className='rounded-md mx-auto max-w-md p-4 mt-8'>
          Copied: {copiedText}
        </span>}
      </div>
    </div>
...
```ts title="use-copy-to-clipboard.ts" icon="ts"
"use client";
