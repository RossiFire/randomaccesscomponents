...
function MultiCodeBlock({ blocks, groupId, defaultValue, children }: MultiCodeBlockProps) {
		...
		<Tabs groupId={groupId} defaultValue={defaultValue ?? resolvedBlocks[0]?.value}>
			...
			{resolvedBlocks.map((block) => (
				<TabsContent value={block.value} key={block.value}>
					<DynamicCodeBlock lang={block.lang} code={block.code} codeblock={block.codeblock} />
				</TabsContent>
			))}
		</Tabs>
	);
